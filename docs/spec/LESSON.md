...
pdf2md 项目从需求确认到 v1.0 实现完成，共经历 22 次提交、16 轮代码审查。通过子代理驱动开发模式（Subagent-Driven Development），每个任务独立实现并经两轮审查（Spec 合规审查 + 代码质量审查），确保了代码质量。后续补充了 34 个自动化测试，覆盖了全部核心函数和集成流程。v2.0 新增分页 API 调用和 LLM 摘要功能，测试数增加到 41 个。
...
## 3.10 函数移除时应同步删除测试和导入

**问题**: v2.0 中 `file_to_base64()` 和 `call_api()` 被移除后，`base64` 模块导入也应移除，对应测试类需一并删除。

**教训**: 重构时应检查被删除函数的所有引用（包括 import 语句和测试），避免残留无用代码。

## 3.11 分页终止条件需要清晰定义

**问题**: 分页循环中需要明确判断何时停止（API 返回 None / 空结果 / 空 content）。

**教训**: 设计分页逻辑时，应在入口处明确约定终止条件并在文档中说明，避免隐式假设。当前使用 `results` 为空作为终止信号。

## 3.12 集成测试中 mock 响应数需与调用次数匹配

**问题**: v2.0 集成测试中，`mock_post.side_effect` 需要为每次 API 调用（分页 + 摘要）提供响应，漏掉一个会导致 `StopIteration`。

**教训**: 测试中模拟多轮调用时，应数清楚实际调用次数，或使用 `MagicMock(return_value=...)` 而非 `side_effect` 列表来避免计数错误。
